...
const URL_ = process.env.URL || "http://127.0.0.1:8123/video/franchise-growth/index.html#capture";
const FPS = 30, DUR = 60;
...
if (stills) {
  ...
  const ff = spawn(process.env.FFMPEG || "ffmpeg", ["-y", "-loglevel", "error", "-f", "image2pipe", "-framerate", String(FPS), "-i", "-",
    "-c:v", "libx264", "-preset", "slow", "-crf", "16", "-pix_fmt", "yuv420p", "-movflags", "+faststart", "frames.mp4"], { stdio: ["pipe", "inherit", "inherit"] });
  const total = FPS * DUR;
